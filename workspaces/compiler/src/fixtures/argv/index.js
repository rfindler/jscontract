...
module.exports = self = {
    ...
    path: function (value) {
      ...
      if (rhome.exec(value)) {
        value = PATH.normalize(
          process.env.HOME + "/" + value.replace(rhome, "")
        );
  ...
  option: function (mod, object) {
    ...
    // Handle edge case
      ...
      throw new Error(
        "No option definition provided" + (mod ? " for module " + mod : "")
      );
    }
    ...
    // Correct the object
      ...
      object.type = self.isFunction(object.type)
        ? object.type
        : self.isString(object.type) && rlistcsv.exec(object.type)
        ? self.types["listcsv-combo"](object.type)
        : self.isString(object.type) && self.types[object.type]
        ? self.types[object.type]
        : self.types.string;
  ...
  help: function (mod) {
    ...
    if (mod && (mod = self.mods[mod])) {
      ...
      for (name in mod.options) {
        ...
        output.push(
          "\t--" + option.name + (option.short ? ", -" + option.short : "")
        );
      ...
      // Shorthand option '-o'
        ...
        if (arg.length > 1) {
          arg.split("").forEach(function (character) {
            ...
            args.options[option.name] = option.type(
              "true",
              option.name,
              args.options,
              args
          ...
          if (
            peek &&
            option.test &&
            option.test(peek, option.name, args.options, args)
          ) {
            value = peek;
            skip = true;
