...
/*=====================================================================*/
/*    serrano/trashcan/accounting.js                                   */
/*    -------------------------------------------------------------    */
/*    Author      :  Manuel Serrano                                    */
/*    Creation    :  Thu May 16 08:58:06 2019                          */
/*    Last change :  Wed Jul  3 10:55:18 2019 (serrano)                */
...
function CTObject( fields ) {
   return new CT( function( info ) {
	 ...
	 get: function( target, prop ) {
	    const ct = ei[ prop ];
/* 	    console.log( "prop=", prop, " ct=", ct );                  */
/* 	    if( prop === "toString" ) process.exit( 1 );               */
	    if( ct ) { 
	       return ct.ctor( target[ prop ] );
	    } else {
	       return target[ prop ];
	    }
      	 },
...
const accounting = (function(root, undefined) {
...
/*    bench                                                            */
/*---------------------------------------------------------------------*/
const ctFormat = CTObject( { pos: isString, neg: isString, zero: isString } );
const ctOpt = CTObject( { symbol: isString, precision: Number.isInteger, format: ctFormat } );
const ct = CTFunction( [ Number.isInteger, ctOpt ], isString ); 
accounting.formatMoneyCT = ct.wrap( accounting.formatMoney );

function test() {
   //accounting.formatMoney(12345678, {} ) === "$12,345,678.00" 
   return accounting.formatMoneyCT(5318008, { symbol: "GBP",
					   precision: 0,
					   thousand: ".",
					   format: {
					      pos : "%s %v",
...
/*       && accounting.formatMoney([123, 456, [78, 9]], "$", 0).join( ", " ) */
/* 	 === "$123, $456, $78,$9"                                      */
}
...
//console.log( bench( 10000 ) );
console.log( bench( 1000000 ) );
