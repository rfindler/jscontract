  ...
  switch (node.kind) {
    case ts.SyntaxKind.FunctionDeclaration:
      if (node.name) {
        ...
        const exp =
          node.modifies &&
          node.modifiers.find((n) => n.kind === ts.SyntaxKind.ExportKeyword);
    ...
    case ts.SyntaxKind.TypeAliasDeclaration:
      if (node.name) {
        ...
        ns.declarations.push(
          new TypeDecl(id, ns, descr.rec ? `CT.CTRec( () => ${ct} )` : ct)
        );
      }
      break;
  ...
  if (!node.kind) {
    ...
    switch (node.kind) {
      ...
      case ts.SyntaxKind.TypeLiteral:
        return `CT.CTObject( { ${node.members
          .map((s) => sigCT(s, env))
          .join(", ")} } )`;
      case ts.SyntaxKind.UnionType:
        return `CT.CTOr( ${node.types
          .map((t) => typeCT(t, env))
          .filter((x) => x)} )`;
...
function intfCT(node, env) {
  return `CT.CTInterface( { ${node.members
    .map((s) => sigCT(s, env))
    .join(", ")} } )`;
}
...
function sigCT(node, env) {
  ...
  switch (node.kind) {
    ...
    case ts.SyntaxKind.IndexSignature:
      ...
      return `"${nameToString(
        param.name
      )}": { contract: ${ct}, index: "${typeName(param.type, env)}"  }`;
